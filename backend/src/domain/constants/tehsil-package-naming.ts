...
export const TEHSIL_PACKAGE_NAMING: Record<string, TehsilPackageNaming> = {
  BHOWANA: {
    zoneLabel: 'Central-I',
...
export function getTehsilPackageNaming(
  tehsilName: string,
): TehsilPackageNaming | null {
  return TEHSIL_PACKAGE_NAMING[tehsilName.toUpperCase()] ?? null;
}

export function getTehsilDisplayName(tehsilName: string): string {
  const naming = getTehsilPackageNaming(tehsilName);
  if (naming) return naming.displayLabel;
  return titleCase(tehsilName);
}

export function composeProcurementPackageName(
  zoneLabel: string,
...
/** Removes auto-inserted tehsil from a stored package name for editing. */
...
export function getTehsilNamesForGroup(groupKey: string): string[] {
  return Object.entries(TEHSIL_PACKAGE_NAMING)
